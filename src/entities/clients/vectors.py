...
class VectorStoreClient:
    ...
    async def _request_with_retries(
        self, method: str, url: str, is_async: bool = True, **kwargs
    ) -> Any:
        retries = 3
        ...
        for attempt in range(retries):
            try:
                response = (
                    ...
                    if is_async
                    else client.request(method, url, **kwargs)
                )
    ...
    async def create_vector_store(
        ...
        try:
            ...
            logging_utility.error(
                "Qdrant collection creation failed for '%s': %s", collection_name, str(e)
            )
        ...
        try:
            ...
            logging_utility.error(
                "API registration failed for store '%s' (ID: %s). "
            ...
            try:
                ...
                logging_utility.error(
                    "Failed to rollback Qdrant collection '%s': %s",
                    collection_name,
                    str(rollback_error),
                )
    ...
    async def add_file_to_vector_store(
        ...
        try:
            ...
            logging_utility.info(
                "Successfully uploaded chunks to Qdrant for '%s'. Result: %s",
                file_path.name,
                qdrant_result,
            )
        except Exception as e:
        ...
        logging_utility.info(
            "Registering file '%s' (ID: %s) in vector store '%s' via API",
            file_path.name,
            file_record_id,
            vector_store_id,
        )
    ...
    async def delete_vector_store(
        ...
        try:
            ...
            if permanent:
                raise VectorStoreClientError(
                    f"Failed to delete vector store backend: {str(e)}"
                ) from e
    ...
    async def list_store_files(
        ...
        try:
            ...
            return [
                ValidationInterface.VectorStoreRead.model_validate(item) for item in response_data
            ]
        except Exception as api_error:
            logging_utility.error(
    ...
    async def attach_vector_store_to_assistant(
    ...
    async def detach_vector_store_from_assistant(
        self, vector_store_id: str, assistant_id: str
    ) -> bool:
        logging_utility.info(
        ...
        response = await self._request_with_retries(
            "DELETE", f"/v1/assistants/" f"{assistant_id}/vector-stores/{vector_store_id}/detach"
        )

        return bool(response)

    async def get_vector_stores_for_assistant(
    ...
    async def retrieve_vector_store_by_collection(
