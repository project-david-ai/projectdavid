...
class VectorStoreFileUpdateStatusInput(BaseModel):
    status: ValidationInterface.StatusEnum = Field(..., description="The new status for the file record.")
...
class VectorStoreClient:
    ...
    def close(self):
        ...
        try:
            ...
            if loop.is_running():
                ...
                return
        except RuntimeError: pass # No loop running
        try:
    ...
    async def _internal_search_vector_store_async(
        ...
        try:
            ...
            logging_utility.error(f"Vector store {vector_store_id} not found via API: {e}"); raise
        ...
        try:
            search_results = self.vector_manager.query_store(store_name=collection_name, query_vector=query_vector, top_k=top_k, filters=filters)
    ...
    async def _internal_delete_vector_store_async(
        ...
        try:
            api_response = await self._internal_request_with_retries("DELETE", f"/v1/vector-stores/{vector_store_id}", params={"permanent": permanent})
            ...
            return {"vector_store_id": vector_store_id, "status": "deleted", "permanent": permanent, "qdrant_result": qdrant_result}
        except Exception as api_error:
    ...
    async def _internal_delete_file_from_vector_store_async(
        ...
        try:
            ...
            api_response = await self._internal_request_with_retries("DELETE", f"/v1/vector-stores/{vector_store_id}/files", params={"file_path": encoded_file_path})
            logging_utility.info("API delete call successful for file record '%s'.", file_path)
            # API returns 204, so api_response will be None. Construct success dict.
            return {"vector_store_id": vector_store_id, "file_path": file_path, "status": "deleted", "qdrant_result": qdrant_result}
        except Exception as api_error:
    ...
    async def _internal_list_store_files_async(
        ...
        try:
            ...
            if not isinstance(response_data, list): raise VectorStoreClientError(f"API returned non-list response for files: {response_data}")
    ...
    async def _internal_update_file_status_async(
        self, vector_store_id: str, file_id: str, status: ValidationInterface.StatusEnum, error_message: Optional[str] = None
    ) -> ValidationInterface.VectorStoreFileRead:
        """Core async logic for updating file status via API."""
        logging_utility.info(f"Updating status for file '{file_id}' in store '{vector_store_id}' to '{status}' via API")
        payload = VectorStoreFileUpdateStatusInput(status=status, error_message=error_message).model_dump(exclude_none=True)
        response_data = await self._internal_request_with_retries(
            "PATCH", f"/v1/vector-stores/{vector_store_id}/files/{file_id}", json=payload
        )
    ...
    def _run_sync(self, coro):
        ...
        try:
            loop = asyncio.get_running_loop()
            if loop.is_running(): raise VectorStoreClientError("Cannot call synchronous method from within an active asyncio event loop.")
        except RuntimeError: pass # No loop running
        return asyncio.run(coro)


    def create_vector_store(
        self, name: str, user_id: str, vector_size: int = 384, distance_metric: str = "Cosine", config: Optional[Dict[str, Any]] = None,
    ) -> ValidationInterface.VectorStoreRead:
    ...
    def add_file_to_vector_store(
        self, vector_store_id: str, file_path: Union[str, Path], user_metadata: Optional[Dict[str, Any]] = None,
    ) -> ValidationInterface.VectorStoreRead:
    ...
    def search_vector_store(
        self, vector_store_id: str, query_text: str, top_k: int = 5, filters: Optional[Dict] = None,
    ) -> List[Dict[str, Any]]:
    ...
    def update_vector_store_file_status(
        ...
        """
        return self._run_sync(self._internal_update_file_status_async(vector_store_id, file_id, status, error_message))
